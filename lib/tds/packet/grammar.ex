defmodule Tds.Packet.Grammar do
  @moduledoc """
  ...
  """
  defmacro byte(n \\ 1), do: quote(do: unsigned - size(8) - unit(unquote(n)))
  ...
  """
  defmacro ushort, do: quote(do: unsigned - 16)
  ...
  @doc """
  An unsigned 4-byte (32-bit) value. The range when used as a
  ...
  defmacro charbin_null(n \\ 2) when n in [2, 4],
    do: quote(do: size(8) - unit(unquote(n)))

  @doc """
  A FRESERVEDBIT is a BIT value used for padding that does not transmit
  information. FRESERVEDBIT fields SHOULD be set to 0b0 and
  **MUST be ignored on receipt**.
  ...
  information. FRESERVEDBYTE fields SHOULD be set to 0x00 and
  **MUST be ignored on receipt**.
  """
  ...
  """
  defmacro unicodechar(n \\ 1), do: quote(do: size(16) - unit(unquote(n)))

  defmacro bigbinary(n), do: quote(do: binary-size(8)-unit(unquote(n)))
