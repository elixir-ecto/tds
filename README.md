...
Tds can work with dates and times in either a tuple format or as Elixir calendar types. This is enabled in the config with `config :tds, opts: [use_elixir_calendar_types: true]`.
...
The `fractional_seconds` should not be confused with the `microsecond` field of a `%Time{}` or `%DateTime{}` struct.
...
You also will need to have the _sqlcmd_ command line tools installed. Setup
